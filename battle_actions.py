    ...
    def perform_search_battle_actions(self, running, stop, run_event=False):
        ...
        if run_event:
            if not self.image_processor.check_and_click_until_found(self.template_images["EVENT_MATCH_SCREEN"], f"{"EVENT_MATCH_SCREEN".replace('_', ' ').title()}", running, stop, max_attempts=10):
                if not self.image_processor.check_and_click_until_found(self.template_images["RANDOM_MATCH_SCREEN"], f"{"RANDOM_MATCH_SCREEN".replace('_', ' ').title()}", running, stop, max_attempts=10):
                    return False
        else:        
            if not self.image_processor.check_and_click_until_found(self.template_images["RANDOM_MATCH_SCREEN"], f"{"RANDOM_MATCH_SCREEN".replace('_', ' ').title()}", running, stop, max_attempts=10):
                return False
        if not self.image_processor.check_and_click_until_found(self.template_images["BATTLE_BUTTON"], f"{"BATTLE_BUTTON".replace('_', ' ').title()}", running, stop, max_attempts=10):
            return False
